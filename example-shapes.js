      ...
      { 
        for( var i = 0; i <= rows; i++ )        // Travel down the curve described by the parameter "points"
        {
          ...
          for( var j = 0; j <= columns; j++ )
          {
            ...
            this.texture_coords.push( vec2( j/columns*1.5, -i/rows*1.5 ) );
          }
        }
...
Declare_Any_Class( "Tri",    // A cube inserts six square strips into its lists.
  {
      ...
      { 
        ...
        Triangle1.prototype.insert_transformed_copy_into( this, [], mult(transform, scale(1, Math.tan(radians(60)), 1)) );
        Triangle1.prototype.insert_transformed_copy_into( this, [], mult(transform, scale(-1, Math.tan(radians(60)), 1)) );


        transform=translation(0,-.5,0);
        transform=mult(transform, rotation(90, 1, 0, 0));
        Triangle2.prototype.insert_transformed_copy_into( this, [], mult(transform, scale(1, Math.tan(radians(60)), 1)) );
        Triangle2.prototype.insert_transformed_copy_into( this, [], mult(transform, scale(-1, Math.tan(radians(60)), 1)) );
